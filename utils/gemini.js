...
- Keep the person recognizable`
  ...
  return stylePrompts[style] || `Transform this portrait into a stylized artistic version while keeping the person recognizable. Use portrait orientation and high quality.`;
};
...
const resizeBufferToTarget = async (buffer) => {
  ...
  try {
    ...
    console.log(`Resized image metadata: ${meta.format} ${meta.width}x${meta.height}`);
    return resized;
  } catch (error) {
...
exports.processImageWithGemini = async (imagePath, style) => {
  try {
      ...
      if (responseText) {
        ...
        for (const pattern of patterns) {
          ...
          if (match) {
            ...
            console.log(`Found base64 data using pattern, length: ${base64Data.length}`);
            ...
            try {
              ...
              console.log('Failed to parse base64 data, trying next pattern...');
            }
          }
        }
      }
    ...
    throw new Error('No image data found in Gemini response. The model may not have generated an image.');
    ...
    if (error.message.includes('API_KEY')) {
      console.error('API Key issue - check your GOOGLE_API_KEY environment variable');
